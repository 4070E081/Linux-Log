...
## facility
用以代表訊息的來源, lcal0 至 local7 為使用者自定.系統預定的有
kern :kernel message
...
在這不會看到 "Script test for debug" 這一段訊息.是因為 debug 的 Level 在 info 之下 (錯誤等級 由低至高 
debug,info,notice,warning,err,crit,alert 與 emerg) 所以不會紀錄至登錄檔.
